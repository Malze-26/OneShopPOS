...
import { Building2, Users, TrendingUp, UserPlus, RefreshCw, CheckCircle, XCircle, Slash, Star, Award } from 'lucide-react';
...
export default function AnalyticsPage() {
  ...
  const getPlanBadge = (plan: string) => {
    const styles: Record<string, string> = { free: 'bg-gray-100 text-gray-800', basic: 'bg-blue-100 text-blue-800', premium: 'bg-purple-200 text-purple-900', enterprise: 'bg-green-100 text-green-900' };
    const n = plan?.toLowerCase() ?? 'free';
    const UseIcon = ['premium', 'enterprise'].includes(n) ? Award : Star;
    return <span className={`px-2 py-1 text-xs font-semibold rounded capitalize ${styles[n] ?? 'bg-gray-100 text-gray-800'}`}><UseIcon className="w-3 h-3 inline mr-1" />{plan || 'free'}</span>;
  };
              ...
              {(computedAlerts as { title: string; description: string; action: string; color: string }[]).map((alert, i) => {
                ...
                return (
                  <div key={i} className={`p-4 rounded-lg border ${bgColors[alert.color] ?? 'bg-gray-50 border-gray-200'}`}>
                    ...
                    <button className="text-sm text-blue-600 hover:text-blue-700 font-medium">{alert.action} →</button>
                  </div>
                );
              })}
            </div>
