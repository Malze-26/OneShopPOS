...
import { Printer, Lock, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export default function DailyZReportPage() {
  ...
  // Fallback if backend reconciliation is missing
  const expectedCash = data?.reconciliation?.expectedCash ?? 0;
      ...
      <div className="max-w-2xl mx-auto bg-white shadow-2xl rounded-xl overflow-hidden border border-[#e4e7ec] print:shadow-none print:border-none print:max-w-full">
            ...
            <div className="flex-1 w-full">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  placeholder="0.00"
                  className="w-full pl-12 pr-4 py-3 bg-white border border-[#c3d1ff] rounded-lg text-xl font-black text-[#101828] focus:ring-4 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all shadow-inner"
                />
              </div>
            </div>
            <button className="w-full md:w-auto px-10 py-3.5 bg-[#0052cc] hover:bg-[#0747a6] text-white rounded-lg font-bold text-sm transition-all shadow-lg hover:shadow-blue-200 flex items-center justify-center gap-2 mt-auto active:scale-95">
        ...
        <div className="p-8 space-y-10">
          ...
          <div className="grid grid-cols-2 gap-y-8 gap-x-12">
            ...
            <InfoBlock label="Register" value={data.shiftInfo?.register || 'Terminal 01'} />
            <InfoBlock label="Cashier" value={data.shiftInfo?.cashier || 'Unknown'} subValue={`ID: ${data.shiftInfo?.cashierId || '---'}`} />
            <InfoBlock label="Shift Opened" value={data.shiftInfo?.openedAt || '--:-- --'} />
          </div>
          ...
          <div>
            ...
            <div className="space-y-4">
              ...
              <SummaryRow label="Tax Collected" value={fmt(data.summary?.taxCollected ?? 0)} currency={currency} />
            </div>
          </div>
          ...
          {/* Reconciliation */}
          <div className="bg-[#f8f9fc] p-6 rounded-2xl border border-[#eaecf0] relative overflow-hidden">
            ...
            <div className="space-y-4">
              <SummaryRow label="Opening Float" value={fmt(data.reconciliation?.openingFloat ?? 0)} currency={currency} />
        ...
        <div className="bg-[#fcfcfd] px-8 py-8 text-center border-t border-[#f0f2f5] space-y-2">
          <p className="text-[9px] text-[#98a2b3] uppercase tracking-[0.3em] font-black">End of Document — OneShop POS System</p>
          <p className="text-[10px] text-[#98a2b3]">Thank you for your business</p>
        </div>
      </div>
    </div>
  );
