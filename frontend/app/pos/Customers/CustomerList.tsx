  ...
  return (
    <div className="bg-white rounded-2xl border border-[#E3E6F0] overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-5 py-4 border-b border-[#E3E6F0]">
        ...
        <button
          onClick={onAdd}
          className="flex items-center gap-2 px-4 py-2 text-white text-[13px] font-semibold rounded-xl hover:brightness-95 transition-all shadow-sm cursor-pointer"
      ...
      <div className="grid grid-cols-[2fr_1.5fr_1fr_1fr_1fr_60px] py-2.5 px-6 bg-[#FAFAFA] border-b border-[#E3E6F0]">
        {["Customer", "Contact", "Orders", "Loyalty Points", "Last Purchase", ""].map((h) => (
          <div key={h} className="text-[12px] font-bold text-[#374151] tracking-[0.3px]">{h}</div>
        ))}
      </div>
      {/* Rows */}
      {filtered.length === 0 ? (
        ...
        filtered.map((c, i) => (
          <div key={c._id} className="grid grid-cols-[2fr_1.5fr_1fr_1fr_1fr_60px] px-6 py-4 border-b border-[#E3E6F0] hover:bg-[#F9FAFB] cursor-pointer" onClick={() => onSelect(c)}>
