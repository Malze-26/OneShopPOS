...
import { getProducts, getCategories, getCustomers, createOrder, updateCustomerPoints } from "../../utils/api";
...
export default function POS() {
  ...
  // Data fetched from backend (fallback to local constants)
  ...
  useEffect(() => {
    ...
    getProducts().then(res => { if (mounted && res?.data) setProducts(res.data); }).catch(err => console.warn('getProducts failed', err));
    getCategories().then(res => { if (mounted && res?.data) setCategories(res.data); }).catch(err => console.warn('getCategories failed', err));
    getCustomers().then(res => { if (mounted && res?.data) setCustomers(res.data); }).catch(err => console.warn('getCustomers failed', err));
...
// ─── Cart Panel ───────────────────────────────────────────────────────────────
function CartPanel({ state, dispatch, subtotal, tax, total, onCheckout, customers }) {
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"100%" }}>
      ...
      <div style={{ padding:"14px 16px 12px", borderBottom:`1px solid ${C.border}`, flexShrink:0 }}>
        ...
        {state.customer ? (
          ...
          <CustomerDropdown customers={customers} onSelect={c=>dispatch({type:"SET_CUST",c})} />
        )}
      </div>
...
// ─── Customer Dropdown ────────────────────────────────────────────────────────
function CustomerDropdown({ onSelect, customers }) {
  ...
  const filteredCustomers = (customers||[]).filter(c=>c.name.toLowerCase().includes(q.toLowerCase()));
      ...
      {open && (
        <div style={{ position:"absolute",left:0,right:0,top:"calc(100% + 6px)",background:"#fff",border:`1.5px solid ${C.border}`,borderRadius:12,boxShadow:"0 8px 32px rgba(0,0,0,.12)",zIndex:50,overflow:"hidden" }}>
          <div style={{ padding:8,borderBottom:`1px solid ${C.border}` }}>
            <input id="customer-search-input" autoFocus value={q} onChange={e=>setQ(e.target.value)} placeholder="Search…" className="input-field" style={{ padding:"7px 12px",fontSize:13 }} />
          </div>
          <ul style={{ maxHeight:160,overflowY:"auto",listStyle:"none" }}>
            ...
            {filteredCustomers.length===0 && <li style={{ padding:"14px",textAlign:"center",fontSize:13,color:C.muted }}>No customers found</li>}
          </ul>
        </div>
      )}
    </div>
  ...
  return (
    <div style={{ marginBottom:10 }}>
      <div style={{ display:"flex",gap:6 }}>
        <input id="promo-input" value={state.promoInput} onChange={e=>dispatch({type:"PROMO_IN",v:e.target.value})} placeholder="Enter promo code" className="input-field"
...
function CheckoutModal({ state, dispatch, subtotal, tax, total, onClose, onSuccess }) {
  ...
  const confirm = async () => {
    try {
      ...
      // Optionally update customer points (best-effort)
      if (state.customer && state.customer.id) {
        const newPts = (state.customer.pts || 0) + Math.floor(total);
        try { await updateCustomerPoints(state.customer.id, newPts); } catch (e) { /* ignore */ }
      }
  ...
  const sendEmail = async () => {
    try {
      // Automatically use customer email if customer is selected
      let to = state.customer?.email;
      
      if (!to) {
        // If no customer selected, prompt for email
        to = window.prompt("Enter recipient email:");
      }
      ...
      const now = new Date();
      const lines = [];
      lines.push("COFFEE & BLOOM");
      lines.push("123 Market Street, Suite 500");
      lines.push("San Francisco, CA 94103");
      lines.push("\n");
        ...
        {step==="pay" ? (
          <div style={{ padding:"18px 20px 20px", display:"flex", flexDirection:"column", gap:16 }}>
            ...
            {method==="cash" && (
              <div>
                ...
                <input id="cash-input" type="number" value={cash} onChange={e=>setCash(e.target.value)} placeholder={total.toFixed(2)} className="input-field" style={{ fontFamily:"'DM Mono',monospace",fontSize:15 }} />
          ...
          <div style={{ padding:"28px 24px 24px", display:"flex", flexDirection:"column", alignItems:"center", gap:16 }}>
            ...
            <div style={{ display:"flex",gap:8,width:"100%" }}>
              ...
              <button className="ghost-btn" style={{ flex:1,justifyContent:"center" }} onClick={sendEmail}>
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M4 4h16v12H4z"/><polyline points="22,6 12,13 2,6"/></svg>
                Send Email
              </button>
