...
// Calculate totals
const totalCash = transactionsData.filter(t => t.payment === 'Cash').reduce((sum, t) => sum + t.amount, 0);
const totalCard = transactionsData.filter(t => t.payment === 'Card').reduce((sum, t) => sum + t.amount, 0);
const totalBank = transactionsData.filter(t => t.payment === 'Bank Transfer').reduce((sum, t) => sum + t.amount, 0);
    ...
    <div className="p-6 max-w-[1400px]">
      ...
      <div className="bg-white rounded-xl p-4 shadow-sm border border-[#e4e7ec] mb-6">
        <div className="flex flex-col md:flex-row gap-3">
          <div className="flex gap-2">
            <button className="px-4 py-2 bg-[#155dfc] text-white rounded-lg text-sm font-medium">
              All
            </button>
            <button className="px-4 py-2 border border-[#e4e7ec] text-[#4a5565] hover:bg-[#f9fafb] rounded-lg text-sm font-medium transition-colors">
              Cash
            </button>
            <button className="px-4 py-2 border border-[#e4e7ec] text-[#4a5565] hover:bg-[#f9fafb] rounded-lg text-sm font-medium transition-colors">
              Card
      ...
      <div className="bg-white rounded-xl shadow-sm border border-[#e4e7ec] overflow-hidden">
        <div className="px-5 py-4 border-b border-[#e4e7ec]">
          ...
          <p className="text-xs text-[#4a5565] mt-1">Showing {transactionsData.length} transactions</p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-[#f9fafb] border-b border-[#e4e7ec]">
              <tr>
                <th className="px-5 py-3 text-left text-xs font-semibold text-[#4a5565] uppercase tracking-wider">
                  Transaction ID
                </th>
                <th className="px-5 py-3 text-left text-xs font-semibold text-[#4a5565] uppercase tracking-wider">
                  Order ID
              ...
              {transactionsData.map((transaction) => (
                <tr key={transaction.id} className="hover:bg-[#f9fafb] transition-colors">
                  <td className="px-5 py-4">
                    <div className="text-sm font-medium text-[#155dfc]">{transaction.id}</div>
                  </td>
                  <td className="px-5 py-4">
                    <div className="text-sm font-medium text-[#101828]">{transaction.orderId}</div>
                  </td>
                  <td className="px-5 py-4">
                    <div className="text-sm text-[#101828]">{transaction.customer}</div>
                  </td>
                  <td className="px-5 py-4">
                    <span
                      className="inline-block px-2 py-1 rounded text-xs font-medium"
                      style={{
                        backgroundColor: paymentConfig[transaction.payment as keyof typeof paymentConfig].bg,
                        color: paymentConfig[transaction.payment as keyof typeof paymentConfig].text,
                      }}
                    >
                      {transaction.payment}
                    </span>
                  ...
                  <td className="px-5 py-4">
                    <div className="text-sm text-[#4a5565]">{transaction.date}</div>
                  </td>
                  <td className="px-5 py-4">
                    <span
                      className="inline-block px-2 py-1 rounded-full text-xs font-medium"
                      style={{
                        backgroundColor: statusConfig[transaction.status as keyof typeof statusConfig].bg,
                        color: statusConfig[transaction.status as keyof typeof statusConfig].text,
                      }}
                    >
        ...
        <div className="px-5 py-4 border-t border-[#e4e7ec] flex items-center justify-between">
          <p className="text-sm text-[#4a5565]">Showing 1 to {transactionsData.length} of 324 transactions</p>
          <div className="flex gap-2">
